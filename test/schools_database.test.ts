...
describe("Schools Database", function () {
    ...
    describe("DbTeacher Database", () => {
        ...
        it("should link a teacher to a school", async () => {
            ...
            const results = await teachersImpl.linkTeacherToSchool(
                teachers[0].id as number,
                schools[0].id as number
            );
        ...
        it("should link teachers to schools", async () => {
            ...
            let schools = await schoolsImpl.getSchools();
            let teachers = await teachersImpl.getTeachers();
            let results = await teachersImpl.linkTeacherToSchool(
                teachers[0].id as number,
                schools[0].id as number
            );
            assert.equal(true, results);
            ...
            schools = await schoolsImpl.getSchools();
            teachers = await teachersImpl.getTeachers();
            results = await teachersImpl.linkTeacherToSchool(
                teachers[1].id as number,
                schools[1].id as number
            );
            assert.equal(true, results);
        });
    });
    ...
    describe("DbLeaners Database", () => {
        ...
        it("should link learners to a school", async () => {
            ...
            let results = await learnersImpl.linkLearnerToSchool(
                learners[0].id as number,
                school[0].id as number
            );
        ...
        it("should change learners to another school", async () => {
            ...
            const linkedLearner = await learnersImpl.linkLearnerToNewSchool(
                learners[0].id as number,
                school[1].id as number
            );
            assert.equal(true, linkedLearner);

            // Test for the previous school
            results = await learnersImpl.linkLearnerToSchool(
                learners[0].id as number,
                school[0].id as number
            );
            assert.equal(false, results);
        ...
        it("should find a learner's current school", async () => {
            ...
            const learnersCurrentSchool = await learnersImpl.getLearnersCurrentSchool(
                learners[0].id as number
            );
            assert.deepEqual(
                { id: school[1].id, name: "Zola Business High" },
        ...
        it("should find all the schools for a learner", async () => {
            ...
            const allPastSchools = await learnersImpl.getPastLearnerSchools(
                learners[0].id as number
            );
    ...
    describe("SchoolSystem Class", function () {
        it("should create schools using SchoolSystem Class", async () => {
            const results = await schoolSystem.createSchools(
                "Luhlaza",
                "Khayelitsha"
            );
            assert.equal(true, results);
        ...
        it("should link teachers to a school using SchoolSystem Class", async () => {
            ...
            let results = await schoolSystem.linkTeacherToSchool(
                teachers[0].id as number,
                schools[0].id as number
            );
            assert.equal(true, results);
            ...
            results = await schoolSystem.linkTeacherToSchool(
                teachers[2].id as number,
                schools[1].id as number
            );
            assert.equal(true, results);
        });
        it("should create a learner using the SchoolSystem Class", async () => {
            const grade = await schoolSystem.createGrade("Grade-12");
            const grades = await schoolSystem.getGrades();

            assert.equal(true, grade);
            assert.equal("Grade-12", grades[0].name);

            const results = await schoolSystem.createLearner({
                firstName: "Lindani",
                lastName: "Mamadi",
                email: "mamadi@gmail",
                gradeId: grades[0].id,
        ...
        it("should link a learner to a new school", async () => {
            const grade = await schoolSystem.createGrade("Grade-10");
            const grades = await schoolSystem.getGrades();

            assert.equal(true, grade);
            assert.equal("Grade-10", grades[0].name);

            const learner = await schoolSystem.createLearner({
                firstName: "Tendani",
                lastName: "Mamadi",
                email: "tendani@gmail",
                gradeId: grades[0].id,
            ...
            const learners = await learnersImpl.getLearners();
            // create a school
            await schoolSystem.createSchools("Cape Town High", "Cape Town");
            const school = await schoolSystem.getSchools();
            let results = await schoolSystem.linkLearnerToSchool(
                learners[0].id as number,
                school[0].id as number
            );
        ...
        it("should find the learner's current school", async () => {
            const grade = await schoolSystem.createGrade("Grade-10");
            const grades = await schoolSystem.getGrades();
            ...
            const learners = await learnersImpl.getLearners();
            // create a school
            await schoolSystem.createSchools("Zola High", "Cape Town");
            const school = await schoolSystem.getSchools();
            let results = await schoolSystem.linkLearnerToSchool(
                learners[0].id as number,
                school[0].id as number
            ...
            const currentSchool = await schoolSystem.getLearnersCurrentSchool(
                learners[0].id as number
            );
            assert.deepEqual(
                {
